...
| **换生成模型零成本** | 以后要上 Ollama 离线模式（P3 的答辩断网保险），只改 `llm.py` 一个文件 |
...
|---|---|
| P0 全部（修库 / 两段式 / 配置化） | ✅ 完成 |
| P1 全部（5 个文件） | ✅ 完成 → **命令行能问答** |
| P2-7 评测集本地跑分 | ✅ 已完成（Recall@5 100%、MRR 0.923） |
| **P2-9 本地 vs 云端 A/B** | ⬜ 下一步：`& $PY "...\检索评测\run_eval.py" --backend dify --profile pure`（要 Dify Key） |
| **P2-10 端到端 5 题与云端 v2 对齐** | ⬜ A/B 之后做 |
| P3 Ollama 全离线 / 界面 | ⬜ 有余力再说 |
